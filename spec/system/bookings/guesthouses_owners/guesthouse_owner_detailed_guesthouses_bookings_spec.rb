...
describe 'Guesthouse sees detailed booking' do
  it 'successfully' do
    # Arrange
    ...
    expect(page).to have_content('Detalhes da Reserva ' + guesthouse[0].rooms.first&.bookings&.first.reservation_code)
    expect(page).to have_content('Situação: Pendente')
    expect(page).to have_content('Quarto: Quarto Aquarela 0')
    expect(page).to have_content('Data de Check-in: ' + guesthouse[0].rooms.first&.bookings&.first.check_in_date.strftime('%d/%m/%Y'))
    expect(page).to have_content('Hora de Check-in: ' + guesthouse[0].rooms.first&.bookings&.first.check_in_hour.strftime('%H:%M'))
    expect(page).to have_content('Data de Check-out: ' + guesthouse[0].rooms.first&.bookings&.first.check_out_date.strftime('%d/%m/%Y'))
    expect(page).to have_content('Hora de Check-out: ' + guesthouse[0].rooms.first&.bookings&.first.check_out_hour.strftime('%H:%M'))
    expect(page).to have_content('Quantidade de Hóspedes: ' + guesthouse[0].rooms.first&.bookings&.first.number_of_guests.to_s)
    expect(page).to have_content('Preço Total: R$ ' + guesthouse[0].rooms.first&.bookings&.first.total_price.to_s)
  end
end