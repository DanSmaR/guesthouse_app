...
describe 'Guesthouse owner visit active guesthouse bookings' do
  it 'and sees them successfully' do
    # Arrange
    ...
    travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 1.days.from_now.day, 14, 0, 0) do
      ...
      # Assert
      expect(current_path).to eq(active_bookings_path)
      ...
      expect(page).to_not have_content(4.days.from_now.strftime('%d/%m/%Y'), count: 1)
      expect(page).to_not have_content(5.days.from_now.strftime('%d/%m/%Y'))
      expect(page).to have_content('R$ 200,00')
      expect(page).to_not have_content('R$ 100,00')
      expect(page).to have_content('0A123AC1')
      expect(page).to_not have_content('0B123AC1')
      ...
      expect(page).to_not have_link('Detalhes', href: booking_path(guesthouse[0].rooms.first&.bookings&.last))
    end
  end

  it 'and sees no active bookings' do
    # Arrange
    cities = %w[Itapetininga Camboriú]
    states = %w[SP SC]
    guesthouses_names = %w[Pousada\ Nascer\ do\ Sol Praiana]
