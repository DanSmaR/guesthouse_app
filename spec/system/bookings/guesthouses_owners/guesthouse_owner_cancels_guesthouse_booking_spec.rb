...
describe 'Guesthouse owner cancels booking' do
  context 'when there is no 2 days past the check-in date' do
    it 'unsuccessfully' do
      ...
      # Assert
      expect(page).to have_content('Não é possível cancelar a reserva com menos de 2 dias de atraso.')
  ...
  context 'when there is 2 days past the check-in date' do
    it 'successfully' do
      # Arrange
      cities = %w[Itapetininga Camboriú]
      states = %w[SP SC]
      ...
      travel_to 5.days.from_now do
        ...
        expect(page).to_not have_content(-3.day.from_now.strftime('%d/%m/%Y'))
        expect(page).to have_content(-1.days.from_now.strftime('%d/%m/%Y'), count: 1)
