...
describe 'Guesthouse owner check in booking' do
  ...
  it 'and sees only the pending bookings' do
    # Arrange
    cities = %w[Itapetininga Camboriú]
    states = %w[SP SC]
    guesthouses_names = %w[Pousada\ Nascer\ do\ Sol Praiana]
    ...
    travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 1.days.from_now.day, 14, 0, 0) do
      ...
      expect(page).to_not have_content('0A123AC1')
      expect(page).to have_content('0B123AC1')
      expect(page).to have_button('Check-In', count: 1)
      expect(page).to have_button('Cancelar', count: 1)
      expect(page).to_not have_link('Detalhes', href: booking_path(guesthouse[0].rooms.first&.bookings&.first))
      expect(page).to have_link('Detalhes', href: booking_path(guesthouse[0].rooms.first&.bookings&.last))
    end
  end

  context 'when trying to check-in before the reserved date' do
    it 'unsuccessfully' do
      ...
      travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 0.days.from_now.day, 14, 0, 0) do
        ...
        expect(page).to have_content('Não é possível realizar o check-in antes da data da reserva')
      end
    end
  end

  context 'when trying to check-in before the guesthouse regular check-in hour' do
    it 'unsuccessfully' do
      # Arrange
      cities = %w[Itapetininga Camboriú]
      states = %w[SP SC]
      ...
      guesthouses_names.each_with_index do |name, index|
        ...
        guesthouse[index].rooms.first&.bookings&.create!([{check_in_date: 1.days.from_now, check_out_date: 2.days.from_now,
                                                           number_of_guests: 2, guest: guest, total_price: 200, status: 0,
                                                           check_in_hour: DateTime.now.beginning_of_day + 1.day + 14.hours, check_out_hour: DateTime.now.beginning_of_day + 1.day + 12.hours,
                                                           reservation_code: "#{index}A123AC1"},
                                                          {check_in_date: 4.days.from_now, check_out_date: 5.days.from_now,
                                                           number_of_guests: 2, guest: guest, total_price: 100, status: 0,
                                                           check_in_hour: '14:00', check_out_hour: '12:00',
      ...
      login_as user[0], scope: :user
      visit guesthouse_owner_bookings_path
      travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 1.days.from_now.day, 13, 0, 0) do
        click_on 'Check-In', match: :first

        # Assert
        expect(current_path).to eq(guesthouse_owner_bookings_path)
        expect(page).to have_content('Hora de Check-in é antes da hora de check-in padrão da pousada')
      end
    end
  end
end