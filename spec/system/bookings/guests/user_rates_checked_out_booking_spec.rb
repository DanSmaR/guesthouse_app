...
describe 'User rates a booking' do
  it 'successfully' do
    # Arrange
    cities = %w[Itapetininga Camboriú]
    states = %w[SP SC]
    ...
    travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 5.days.from_now.day, 12, 0, 0) do
      ...
      click_on 'Minhas Reservas'
      click_on 'Detalhes', match: :first
      select 'Muito Bom = 4', from: 'Nota'
      fill_in 'Comentário', with: 'Gostei muito da pousada'
      click_on 'Enviar'

      # Assert
      expect(page).to have_content('Avaliação adicionada com sucesso!')
      expect(current_path).to eq(booking_path(2))
      expect(page).to_not have_content('Avalie sua estadia')
      expect(page).to_not have_select('Nota', options: ['Selecione uma opção',
                                                    'Péssimo = 0',
                                                    'Ruim = 1',
                                                    'Regular = 2',
                                                    'Bom = 3',
                                                    'Muito Bom = 4',
                                                    'Excelente = 5'])
      expect(page).to_not have_field('Comentário', type: 'textarea')
  ...
  it 'and must fill in all fields' do
    ...
    travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 5.days.from_now.day, 12, 0, 0) do
      ...
      click_on 'Minhas Reservas'
      click_on 'Detalhes', match: :first
      within 'div#review-form' do
        select 'Selecione uma opção', from: 'Nota'
      end
      fill_in 'Comentário', with: 'Gostei muito da pousada'
      click_on 'Enviar'

      # Assert
      expect(page).to have_content('Não foi possível adicionar a avaliação')
      expect(current_path).to eq(booking_path(2))
      expect(page).to have_content('Avalie sua estadia')
      expect(page).to have_select('Nota', options: ['Selecione uma opção',
                                                        'Péssimo = 0',
