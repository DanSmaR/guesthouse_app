...
describe 'Guesthouse owner responds to review' do
  it 'successfully' do
    ...
    travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 10.days.from_now.day, 15, 0, 0) do
      reviews = Review.all
      resposta = 'Obrigado por sua avaliação'
      # Act
      login_as user[0], scope: :user
      visit root_path
      ...
      click_on 'Avaliações'
      click_on 'Responder', match: :first
      fill_in 'Resposta', with: resposta
      ...
      expect(page).to have_content("Resposta: #{resposta}")
    end
  end
  ...
  context 'when user is logged in as guest' do
    it 'can see the answer' do
      ...
      travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 10.days.from_now.day, 15, 0, 0) do
        review = Review.first
        resposta = 'Obrigado por sua avaliação'
        review.update!(response: resposta)
    ...
    it 'can see the rating and answer in booking details' do
      # Arrange
      cities = %w[Itapetininga Camboriú]
      states = %w[SP SC]
      guesthouses_names = %w[Nascer\ do\ Sol Praiana]
      ...
      travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 10.days.from_now.day, 15, 0, 0) do
        ...
        expect(page).to have_content("Hóspede: #{guest.name}")
        expect(page).to have_content("Comentário: #{review.comment}")
        expect(page).to have_content("Nota: Ótimo - 5")
        expect(page).to have_content("Resposta: #{review.response}")
      end
    end

    context 'when user is not logged in' do
      it 'cannot see the answer' do
        # Arrange
        cities = %w[Itapetininga Camboriú]
        states = %w[SP SC]
        guesthouses_names = %w[Nascer\ do\ Sol Praiana]
        ...
        travel_to Time.new(0.year.from_now.year, 0.month.from_now.month, 10.days.from_now.day, 15, 0, 0) do
          ...
          # Assert
          expect(page).to_not have_content("Resposta #{resposta}")
        end
      end
    end
  end
